...
accept_charset(State) ->
    ...
    ; false ->
        % item 894
        ChosenCharset = 
            hedgy_util:choose_charset(CharsetsAvailable, CheckCharset),
        % item 888
...
accept_content_encoding(State) ->
    ...
    ChosenContentEncoding = 
        hedgy_util:choose_encoding(EncodingsProvided, CheckEncoding),
    % item 962
    ...
    ; false ->
        ...
        S2 = case ChosenContentEncoding of
            <<"identity">> -> 
                S1;
            _ ->
                Ex1 = hx:set_resp_header(<<"Content-Encoding">>, 
...
accept_content_type(State) ->
    ...
    case AcceptHeader =:= undefined of true -> 
        ...
        S2 = with_exchange(fun(Ex) ->
                Ex1 = hx:set_resp_content_type(Ct, Ex),
                hx:set_resp_content_fun(Cf, Ex1) 
             end, S1),
        {true, ContentTypesProvided, S2}
    ; false ->
        % item 905
        ChosenMediaType = 
            hedgy_util:select_media_type(ContentTypesProvided, AcceptHeader),
        % item 906
        ...
        ; false ->
            ...
            S2 = with_exchange(fun(Ex) ->
                    Ex1 = hx:set_resp_content_type(MediaType, Ex),
                    hx:set_resp_content_fun(Fun, Ex1) 
                 end, S1),
...
call(Name, State) ->
    % item 1110
    Result = hedgy_controller:call(Name, State),
    % item 1111
    {Answer, S1} = Result,
    % item 1126
...
can_range(ETag, LastModified, IfRange) ->
    ...
    ; false ->
        ...
        case is_etag(IfRange) of true -> 
            % item 1188
            ETags = hedgy_util:split_quoted_strings(IfRange),
            % item 1189
                ...
                case ContentEncodingAccepted of true -> 
                    ...
                    ; false ->
                        ...
                        S6 = set_resp_header(<<"Vary">>, 
                            hedgy_util:binary_join(Variances, <<", ">>), S5),
                        {true, S6}
                    end
                ; false ->
                    % item 949
                        ...
                        ; false ->
                            ...
                            ; false ->
                                ...
                                case is_binary(IsAuthorized) of true -> 
                                    % item 986
                                    % item 684
                                    Ex = hx:set_resp_header(<<"WWW-Authenticate">>, 
...
exchange(State) ->
    % item 664
    State#hedgy_flow_state.exchange
.
                                ...
                                case IMS =/= undefined of true -> 
                                    ...
                                    case LastModified =/= undefined of true -> 
                                        ...
                                        ; false ->
                                            ...
                                            % item 1153
                                            RespFun = hx:get_resp_content_fun(exchange(S7)),
...
has_resp_body(State) ->
    % item 652
    hx:has_resp_body(exchange(State))
.
...
match_etag(ETag, HeaderVal) ->
    ...
    ; false ->
        % item 993
        ETags = hedgy_util:split_quoted_strings(HeaderVal),
        % item 994
        lists:member(ETag, ETags)
    end
.
...
options_flow(State) ->
    ...
    Exc = exchange(State),
    Exc1 = hx:set_resp_headers(Hdrs, Exc),
...
parse_http_date(Value) ->
    % item 1179
    Date = try hedgy_http_date:parse(Value) of
            V -> V
        catch
            _:_ -> {error, catched}
    end
...
request(State) ->
    % item 646
    Exchange = State#hedgy_flow_state.exchange,
    Exchange#hedgy_exchange.req
.
...
select_content_type(State) ->
    ...
    SelectedContentType = hedgy_util:select_content_type(ContentTypesAccepted, ContentTypeHeader),
...
set_allowed_methods(AllowedMethods, State) ->
    ...
    HeaderValues = hedgy_util:binary_join(BinMethods, <<", ">>),
...
set_content_type(State) ->
    ...
    with_exchange(fun(Exchange) ->
            CType = hx:get_resp_content_type(Exchange),
            CSet = case hx:get_resp_chosen_charset(Exchange) of
                undefined -> <<"">>;
                Cs -> <<"; charset=", Cs/binary>>
            end,
            hx:set_resp_header(<<"Content-Type">>, 
...
set_date_header(Header, Date, State) ->
    ...
    ; false ->
        ...
        ; false ->
            % item 1034
            V = hedgy_util:rfc1123_date(Date)
        end,
        % item 1033
...
set_exchange(Exchange, State) ->
    % item 634
    State#hedgy_flow_state{exchange=Exchange}
.
...
set_response_code(Code, State) ->
    % item 640
    Ex = hx:set_resp_code(Code, exchange(State)),
    set_exchange(Ex, State)
.

with_exchange(Fun, State) ->
    % item 622
    Exchange = Fun(State#hedgy_flow_state.exchange),
    State#hedgy_flow_state{exchange=Exchange}
.
...
set_resp_chosen_charset(Charset, State) ->
    Ex = hx:set_resp_chosen_charset(Charset, exchange(State)),
    set_exchange(Ex, State).
    
set_resp_header(Header, Value, State) ->
    Ex = hx:set_resp_header(Header, Value, exchange(State)),
    set_exchange(Ex, State).

get_resp_header(Header, State) ->
    {hx:get_resp_header(Header, exchange(State)), State}.

encode_body_if_set(State) ->
    State.

set_header_if_not_set(Header, Value, #hedgy_flow_state{exchange=Ex}=State) ->
    Ex1 = case hx:get_resp_header(Header, Ex) of
        undefined ->
            hx:set_resp_header(Header, Value, Ex);
        _ -> Ex
    end,
