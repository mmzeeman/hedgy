...
-spec make_reqdata(Req :: elli:req()) -> elli_machine:exchange().
make_exchange(Req) ->
    #machine_reqdata{req=Req}.

% @doc Return an elli response.
response(#machine_reqdata{resp_code=Code, resp_headers=Headers, resp_body=Body}) 
        when Code =/= undefined ->
    {Code, Headers, Body}.

% @doc Return the controller
controller(#machine_reqdata{controller=Controller}) ->
    Controller.
...
controller_do(Fun, #machine_reqdata{}=ReqData) ->
